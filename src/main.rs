...
use controllers::gambling::{self, GambleComamnds};
...
fn scheme() -> UpdateHandler<Box<dyn std::error::Error + Send + Sync + 'static>> {
    let general_handler = Update::filter_message().branch(filter(|msg: Message| {
        println!("msg");
        true
    }));
    // todo!("impl this to check user username and update it to take resp from inline handler");
    ...
    let gamble_commands = teloxide::filter_command::<GambleComamnds, _>().branch(
        dptree::case![GuessState::Start]
            .branch(dptree::case![GambleComamnds::Guess].endpoint(gambling::guess::guess_bid)),
    );
