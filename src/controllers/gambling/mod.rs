...
pub mod inline {
    ...
    pub async fn inline_gamble(bot: Bot, q: &InlineQuery) -> anyhow::Result<()> {
        ...
        let pigrace_msg = format!("Смысл игры в том, чтобы поставить деньги на какую-то из свиней. Если выбранная вами свинья пройдет наибольшее расстояние, то вы получите денежный приз - <ставка> * {}$\n\nЧтобы начать игру введите /pigrace. После ввода нужно будет ввести ставку и айди свиньи, ответив на сообщение бота, после чего победитель определится в реальном времени (На любом этапе игры можно ответить 'отмена', остановив таким образом игру).", pigrace::RACE_BID_MULTIPLIER);
